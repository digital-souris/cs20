...
var Cart = {
	...
	nextPage: function nextPage(page) {
		...
		setTimeout(function () {
			$(document).find('.btn-md').trigger('click');
		}, 500);
...
// капча регистрации
  $("#register").on('submit', function (e) { 
    e.preventDefault();
	let obj = this;    
    // var test = obj.querySelector('[name="g-recaptcha-response"]');
    var token = $("#g-recaptcha-response").val();
    // console.log(test);
    // grecaptcha.execute('6LeoEHsaAAAAANKlZJnt4Cn7e6RtHAceAEGZB_1i')
    // .then(function(token){
    if(token){
        $.ajax({
	      dataType: 'json',
    	...
    	alert('Пройдите проверку');
    }
    // });
  });


// капча гарантийного случая

   $(".garanty__form pedit").on('submit', function (e) { 
    e.preventDefault();
	let obj = this;    
   var token = $("#g-recaptcha-response").val();
   	if(token){
